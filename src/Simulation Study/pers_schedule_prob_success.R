...
#                                length.out = lengthout)[-1]
        ...
        print(paste(pDynSurvTime(minSurv, patientDsList[[i]]), lengthout, which.max(probArr), which.max(infoArr), sep="-"))
      ...
      print("Next Patient")
      save(patientDsList, file = paste("Rdata/u3_6Obs_pt05Risk.Rdata", sep=""))
    }else{
