pDynStopTime <- function (object, newdata, Dt, K = 5, M = 500, idVar = "id", maxRiskTime = 0.5,
    ...
    info.times = foreach(ti=1:ntimes,.combine='cbind', .export = c("idT", "fastSumID", "log.p_Tj"),
        ...
        for (m in seq_len(M)) {
            ...
            log.S_timaxRisk <- S.b(times[ti]+maxRiskTime, b.new2, i = 1, ModelMats(times[ti]+maxRiskTime, 1), log = TRUE)
            info[m] <- log.S_timaxRisk - log.S_ti
            #log.S_Tj <- S.b(Tj, b.new2, i = 1, ModelMats(Tj, 1), log = TRUE)
            #log.h_Tj <- logh.b(b.new2, hMats(Tj))
...
environment(pDynStopTime) <- asNamespace('JMbayes')