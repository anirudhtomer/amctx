dynInfo_mod <- function (object, newdata, Dt, K = 5, M = 500, idVar = "id", 
    ...
    b.old3 <- b.new3 <- modes.b
    mcmc <- object$mcmc
    mcmc <- mcmc[names(mcmc) != "b"]
    samples <- sample(nrow(mcmc$betas), 4 * M, replace = TRUE)
    ...
    proposed.b3 <- rmvt(n = M, df = 4, mu = modes.b, Sigma = Vars.b)
    dmvt.proposed3 <- dmvt(proposed.b3, mu = modes.b, Sigma = Vars.b, df = 4, log = TRUE)    
    ...
    info.times = foreach(ti=1:ntimes,.combine='cbind', .export = c("idT", "fastSumID", "log.p_Tj"),
        ...
        for (m in seq_len(M)) {
          print(m)
            ...
            # This was originally [T_j^* | T_j > t, {Y_j(t), y_j(u)}] but we threw all T_J^* which were < u
            ...
            # Step 3-2: Simulate b from [b | T_j > u, {Y_j(t), y_j(u)}]
            p.b2 <- proposed.b2[m, ]
            ...
            b.old2 <- b.new2
            # Step 3-3: Calculate p(T_j | T_j > u, b)
            ...
            ##################################################################################
            # Step 4-1: Simulate parameter values from [theta | D_n]
            betas.new <- mcmc$betas[m + 3*M, ]
            if (hasScale)
            ...
            if (estimateWeightFun)
                shapes.new <- mcmc$shapes[m + 3*M, ]
            D.new <- mcmc$D[m + 3*M, ]; dim(D.new) <- dim(D)
            Bs.gammas.new <- mcmc$Bs.gammas[m + 3*M, ]

            #Step 4-2: Simulate b from [b | T_j >u, Y_j(t)]
            id <- id.
            y <- yy
            X <- XX
            Z <- ZZ
            ...
            b.old3 <- b.new3
            # Step 4-3: Calculate p(T_j | T_j > u, b)
            log.S_ti <- S.b(times[ti], b.new3, i = 1, survMats[[1 + ti]], log = TRUE)
            log.S_Tj <- S.b(Tj, b.new3, i = 1, ModelMats(Tj, 1), log = TRUE)
            log.h_Tj <- logh.b(b.new3, hMats(Tj))
            info[m] <- info[m] - (log.h_Tj + log.S_Tj - log.S_ti)
        }
        #print(count/M)
        #print(count.b/M)
        #print(count.b1/M)
...
environment(dynInfo_mod) <- asNamespace('JMbayes')
