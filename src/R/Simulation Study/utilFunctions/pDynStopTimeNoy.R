pDynStopTimeNoy <- function (object, newdata, Dt, K = 5, M = 500, idVar = "id", maxRiskTime = 0.5,
    ...
    info.times = foreach(ti=1:ntimes,.combine='cbind', .export = c("idT", "fastSumID", "log.p_Tj"),
        ...
        for (m in seq_len(M)) {
            ...
            a <- min(exp(log.posterior.b(p.b, y, list(survMats[[1 + ti]]), ii = 1) + dmvt.old - 
                             log.posterior.b(b.old, y, list(survMats[[1 + ti]]), ii = 1) - dmvt.prop), 1)
            ind <- runif(1) <= a
            if (!is.na(ind) && ind) {
                b.new <- p.b
                count.b <- count.b + 1
            ...
            log.S_ti <- S.b(times[ti], b.new, i = 1, survMats[[1 + ti]], log = TRUE)
            log.S_timaxRisk <- S.b(times[ti]+maxRiskTime, b.new, i = 1, ModelMats(times[ti]+maxRiskTime, 1), log = TRUE)
            info[m] <- exp(log.S_timaxRisk - log.S_ti)
            #log.S_Tj <- S.b(Tj, b.new2, i = 1, ModelMats(Tj, 1), log = TRUE)
            #log.h_Tj <- logh.b(b.new2, hMats(Tj))
...
environment(pDynStopTimeNoy) <- asNamespace('JMbayes')